...
env = gym.make('FrozenLake-v1', desc=$default_map, map_name="$map_name", is_slippery=True).unwrapped  # Access the core environment
"""
...
for state in keys(P_dic)
    for action in keys(P_dic[state])
        for (probability, next_state, reward, ter) in P_dic[state][action]
            ...
            if reward>0
                R[state+1, action+1] += reward*probability
            end
...
##FIB
FIB_solver = FIBSolver(max_iterations = 1000)
start = time()
policy = solve(FIB_solver, tabular_pomdp)
