...
def valueEvaluation(policy, states, actions, transition, gamma, window_len,val_terminal):
    v0 = {state: 0 for state in states}
    v1 = {state: 0 for state in states}
    for state in val_terminal:
        v1[tuple([state])] = val_terminal[state]
        v0[tuple([state])] = val_terminal[state]
    max_error = 1e-10
    count = 0
    a0 = 0
    while(1):
        ...
        error = -1
        for state in states:
            if state[0] in val_terminal and len(state)==1:
                continue
            v1[state] = 0
...
def brute_force_search(states, actions, transition, gamma, window_len, val_term = {}):
    policy = {state: list(transition[state].keys())[0] for state in states}
    while(True):
        V = valueEvaluation(policy, states, actions, transition, gamma, window_len, val_term)
        improved_policy = policy.copy()
        improvable = False
        for s in states:
            if s[0] in val_term and len(s)==1:
                continue
            for a in actions:
                if a == improved_policy[s] or a not in transition[s].keys():
                    continue
                t_policy = policy.copy()
                if(V[s] - Q_pi(V, s, a, transition, gamma) > 1e-10):
                    improved_policy[s] = a
                    improvable = True
                    break
        if not improvable:
        ...
        # print(policy)
    # print("out of while")
    value_function = valueEvaluation(policy, states, actions, transition, gamma, window_len, val_term)
    policy = policy
